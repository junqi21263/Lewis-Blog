...
function uniqueByUrl(images: FeaturedImage[]) {
...
export const onRequestGet: PagesFunction<Env> = async (context) =>
  await withErrorHandling(async () => {
    ...
    const limit = Math.min(12, Math.max(1, Number(url.searchParams.get("limit") || 9)));
      ...
      context.env.DB
        .prepare(
          `SELECT id, title, description, image_url, alt_text
           FROM photos
           WHERE featured = 1 AND image_url IS NOT NULL AND image_url != ''
    ...
    const primary = [...galleryImages.slice(0, 5), ...articleImages.slice(0, 4)];
    const fallback = [...galleryImages.slice(5), ...articleImages.slice(4)];
    const data = uniqueByUrl([...primary, ...fallback]).slice(0, limit);
